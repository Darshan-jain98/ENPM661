...
def In_clearence(row,col):
    ...
    #To check the point is in Hexagon or not
    if col <= 240 and col >= 160 and (5053*col + 8750*row - 1918903 >=0) and (5053*col - 8750*row - 102296 <=0) and (2014*col +3500*row - 1089425 <=0) and (2014*col - 3500*row + 283825 >=0):
        inHexagon =True
    else:
        inHexagon =False
    
    #To check the point is in Quad or not
    if ((25*col + 79*row -5620>=0) and (6*col + 7*row -1016<=0) and (85*col - 69*row + 1972 >=0)) or ((25*col + 79*row -5620>=0) and (16*col - 5*row - 1013<=0) and (85*col - 69*row + 1972 >=0)):
        inQuad = True
    else:
        inQuad = False
    
    #To check if the point is in boundry or not
    if(row <5 or row >244 or col <5 or col >394):
        inBoundry = True
    else:
        inBoundry = False
...
def Move_left(node):
    if (node[1] !=0 ):
        if (not In_clearence(node[0],node[1]-1)):
...
def Move_up(node):
    if(node[0] !=0):
        if (not In_clearence(node[0] - 1, node[1])):
            new_node = np.copy(node)
            new_node[0] = node[0] - 1
            return True, tuple(new_node)
        else:
...
def Move_upleft(node):
    if(node[0] !=0 and node[1] !=0):
        if(not In_clearence(node[0]-1, node[1]-1)):
            new_node = np.copy(node)
            new_node[0] = node[0] - 1
            new_node[1] = node[1] - 1
            return True,tuple(new_node)
...
def Move_downright(node):
    if(node[0] != 249 and node[1] != 399):
        if(not In_clearence(node[0]+1,node[0]+1)):
            new_node = np.copy(node)
            new_node[0] = node[0] + 1
            new_node[1] = node[1] + 1
            return True, tuple(new_node)
...
while True:
    print("All the values should be between (5,394) for x and (5, 244) for y with origin at bottom right corner")
    ...
    Goal_row = 249 - goal_y
    Goal_col = goal_x
    if not In_clearence(Initial_row,Initial_col) and initial_x>=5 and initial_x<=394 and initial_y>=5 and initial_y<=244:
        if not In_clearence(Goal_row,Goal_col) and goal_x>=5 and goal_x<=394 and goal_y>=5 and goal_y<=244:
            break
...
while True:
    ...
    if (not In_clearence(current_node[0],current_node[1])):
...
for i in range(len(close_lst)):
    if In_clearence(close_lst[i][0][0],close_lst[i][0][1]):
        viz_matrix[close_lst[i][0][0],close_lst[i][0][1]] = (0,0,0)
        continue
